...
$spreadsheet->setActiveSheetIndex(0)
            ->setCellValue('A1', 'Variables Contextuales de Ingreso')
...
$sql="SELECT deno, count(*) as cantidad FROM `sujetos` left join tablas on ctex1=valo and tipo='CTEX'
WHERE legajo in (select distinct admi_legajo from hogares_admision 
 ...
 group by deno	
 union all 
SELECT deno, count(*) as cantidad  FROM `sujetos` left join tablas on ctex2=valo and tipo='CTEX'
WHERE legajo in (select distinct admi_legajo from hogares_admision 
 left join dispositivos on admi_hogar=dispositivos.id where deno is not null and ".$condicion." and admi_alta<=".fsql($_GET["hasta"])." and (admi_baja is null or admi_baja>=".fsql($_GET["desde"]).")) 
 group by deno
 union all  
SELECT deno, count(*) as cantidad FROM `sujetos` left join tablas on ctex3=valo and tipo='CTEX'
WHERE legajo in (select distinct admi_legajo from hogares_admision 
 left join dispositivos on admi_hogar=dispositivos.id where deno is not null and ".$condicion." and admi_alta<=".fsql($_GET["hasta"])." and (admi_baja is null or admi_baja>=".fsql($_GET["desde"]).")) 
 group by deno
 order by deno"; 
...
$sindatos=un_campo("select count(distinct admi_legajo) from hogares_admision 
 ...
 where (not ctex1>0 and not ctex2>0 and not ctex3>0) and ".$condicion." and admi_alta<=".fsql($_GET["hasta"])." and (admi_baja is null or admi_baja>=".fsql($_GET["desde"]).")");
...
$spreadsheet->getActiveSheet()->setTitle('ContextualesIngresos');
$spreadsheet->setActiveSheetIndex(0);
$filename = 'Contextuales-ingresos.xlsx';
