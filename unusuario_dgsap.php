...
$usua= $_GET["vusuario"];
$conn =registros("select id, denominacion from sectores where baja is null and (id=2 or dependencia=2 or dependencia in (1,2,124)) order by denominacion");
$opci ="";
...
$oppe=$oppe."<option value='".$re['id']."'>".$re['denominacion']."</option>";};
  ...
  where usuarios.id=".$usua." and (sectores.id=2 or dependencia=2 or dependencia in (1,2,124))";
	$conn=registros($sql);;
...
<form class="form" method='post' action='usuario_dgsap_actualiza' onsubmit='return valida()' enctype='multipart/form-data'>
