...
$_SESSION["prestacion"]="Proceso de Egreso";
$nya=un_registro("select * from alojados where idalojados=".$nnya);
include("encabezado.php");
$trimestre=$_SESSION["trimestre"];
$anio=$_SESSION["anio"];
$trimestral=un_campo("select trimestral from trim_egreso where legajo=".$nnya." and hogar=".$hogar." and trimestre=".$trimestre." and anio=".$anio);
if($trimestral>"0"){$tri=un_registro("select * from trim_egreso where trimestral=".$trimestral);}
else{$tri=un_registro("select * from trim_egreso where legajo=".$nnya." and hogar=".$hogar." order by anio desc, trimestre desc limit 1");};
?>
</div>
<div class="container">
<p class="text-warning">Informar las acciones que se est&aacute;n desarrollando para tal fin. En el caso de que se encuentre en evaluaci&oacute;n informar las acciones para poder definir la estrategia en el menor tiempo posible.
</p>
<form method="POST" action="egreso_do" onsubmit="return guardar()">
<ul class="list-group">
