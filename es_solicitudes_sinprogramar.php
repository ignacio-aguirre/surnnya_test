...
<div class="container">
<strong>Solicitudes Pendientes sin programar</strong>
...
<?php
 ...
 where fecha_inicio is null and fecha_rechazo is null and (select count(*) from es_acciones where solicitud=es_participaciones.id)=0 order by apellidos,nombres,fecha_ingreso asc";
   $reg =registros($sql);
   $conta=0;
  ...
  "</td><td>".$r["especialidad"]."</td><td>".$r["apellido"]."</td><td>";
      if($r["profesional"]!="") {echo "<btn class='btn-sm btn-success' onclick='accion(".$r["id"].")'>+Acc</btn>&nbsp;";}
      else{echo "<btn class='btn-sm btn-success' onclick='prof(".$r["id"].")'>Profesional</btn>&nbsp;";};
...
<?php echo $conta." solicitudes no programadas" ?>
</div>
