...
function PoneAncho(x){

  if (document.layers||izq(Navegador(),13)=='Google Chrome'){

	 ancho = window.innerWidth-20;

  } else {

    ancho = document.body.clientWidth-15;
  ...
  cosa.style.width=ancho;

};



function PoneAncho2(x){

  if (document.layers||izq(Navegador(),13)=='Google Chrome'){

	 ancho = window.innerWidth-35;

  } else {

    ancho = document.body.clientWidth-30;
...
function chequea_estado(){
  ura="ej_sistema?tipo=ESTADO_SESION";
  pea = new XMLHttpRequest();  
  pea.open('GET', ura, false);
  pea.send(null);
  ok=pea.responseText;
  pea.close;
  if(ok=="0") navega("salir");
return true;
}
