...
$perf="";
$per = registros("select id, denominacion from perfiles where denominacion like 'DGSAP%' order by denominacion");
...
<div class="container">
<form class="form-inline" method='post' action='usuario_dgsap_nuevo_do' onsubmit='return valida()'>
...
if(resp.indexOf("BAJA")>0){
  if(confirm(resp+". Restauras el usuario?")){
    navega("usuario_dgsap_restaura?cuil="+document.getElementById('cuil').value);
  };
};
